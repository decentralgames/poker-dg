...
const cardMapper: (card: { rank: CardRank; suit: CardSuit }) => Card = (
  card
) => ({
  // @ts-ignore
...
const automaticActionFlagToStringArray = (
  automaticActionFlag: AutomaticActionFlag
): AutomaticAction[] => {
...
const stringToAutomaticActionFlag = (
...
export default class Poker {
  ...
  forcedBets(): { ante: number; bigBlind: number; smallBlind: number } {
    const {
      ante = 0,
      blinds: { big: bigBlind, small: smallBlind },
    } = this._table.forcedBets();
    return {
      ante,
      smallBlind,
      bigBlind,
  ...
  playerCards(seat: number): any {
    ...
    return [...this._table.communityCards().cards(), ...holeCards];
  }
  ...
  winners(): [
    SeatIndex,
    { cards: Card[]; ranking: HandRanking; strength: number },
    Card[]
  ][][] {
    return this._table.winners().map((potWinners) =>
      potWinners.map((winner) => {
  ...
  setAutomaticAction(seatIndex: number, action: AutomaticAction | null): void {
    const automaticAction =
      action === null ? action : stringToAutomaticActionFlag(action);
