...
export default class Table {
  ...
  startHand(seat?: number): void {
    ...
    this._handPlayers = this._tablePlayers.map((player) =>
  ...
  holeCards(): (HoleCards | null)[] {
    assert(
      this.handInProgress() || this.bettingRoundsCompleted(),
  ...
  setAutomaticAction(seat: SeatIndex, action: AutomaticAction | null) {
    ...
    assert(
      action === null || action & this.legalAutomaticActions(seat),
  ...
  sitDown(seat: SeatIndex, buyIn: Chips): void {
    assert(
      seat < this._numSeats && seat >= 0,
      'Given seat index must be valid'
    );
    assert(
      this._tablePlayers[seat] === null,
      'Given seat must not be occupied'
    );
  ...
  takeAutomaticAction(automaticAction: AutomaticAction): void {
    ...
    switch (automaticAction) {
      ...
      case AutomaticAction.CHECK_FOLD:
        return this._dealer.actionTaken(
          betGap === 0 ? Action.CHECK : Action.FOLD
        );
      ...
      case AutomaticAction.CALL_ANY:
        return this._dealer.actionTaken(
          betGap === 0 ? Action.CHECK : Action.CALL
        );
  ...
  private incrementButton(): void {
    ...
    if (this._buttonSetManually) {
      ...
      const seat = this._handPlayers
        .slice(offset)
        .findIndex((player) => player !== null);
      this._button =
        seat !== -1
          ? seat + offset
